...
export const DEFAULT_VIEW: ViewTransform = { s: 1, x: 0.5, y: 0.5 };
...
export interface AsciiSettings {
  enabled: boolean;
  /** Cell size in 1/1000ths of canvas width, so exports scale correctly. */
  size: number;
  /** 0..1 */
  opacity: number;
  /** 0..1; how deep into the mid-tones glyphs reach. */
  density: number;
  /** 0..1 S-curve on the tonal drive; 0.2 ≈ neutral. */
  contrast?: number;
  /** Glyph vocabulary. */
  set?: AsciiSet;
}
...
export interface Settings {
  ...
  /** 0..1 */
  grain: number;
  /** Texture character; defaults to film. */
  grainType?: GrainType;
  /** 0..1 */
  softness: number;
  /** 0..1 */
  vignette: number;
  /** Pan/zoom camera; defaults to DEFAULT_VIEW. */
  view?: ViewTransform;
  ascii: AsciiSettings;
}
...
function anchor(cols: string[], mode: Mode, rand: () => number): string {
  const sorted = [...cols].toSorted((a, b) => luminance(a) - luminance(b));
  const [first] = sorted;
  const last = sorted.at(-1);
  if (!first || !last) {
    throw new Error("A gradient requires at least one color");
  }
  if (mode === "light") {
    return last;
  }
...
function paintField(
  ...
  const fillAll = () =>
    o.fillRect(-ox - 2, -oy - 2, lw + ox * 2 + 4, lh + oy * 2 + 4);
  ...
  switch (style) {
    case "image": {
      if (!imageField) {
        throw new Error("Image style requires an image field");
      }
    ...
    case "mesh": {
      ...
      for (const b of shuffled(blobs, rand)) {
        blob(b.c, b.x, b.y, b.r);
      }
      break;
    }
    case "waves": {
      ...
      const [firstColor] = order;
      o.fillStyle = firstColor;
      fillAll();
      ...
      for (let i = 0; i < bands.length; i += 1) {
        ...
        const tOpaque = Math.min(
          0.92,
          (opaqueY - top) / Math.max(4, nextY - top)
        );
        ...
        o.moveTo(
          -ext,
          b.yBase + Math.sin(-ext * b.freq + b.phase) * b.amp - feather
        );
        for (let x = -ext; x <= lw + ext; x += 1) {
          o.lineTo(
            x,
            b.yBase + Math.sin(x * b.freq + b.phase) * b.amp - feather
          );
        }
    ...
    case "set": {
      ...
      rest.sort(
        (a, b) => Math.abs(luminance(b) - bgL) - Math.abs(luminance(a) - bgL)
      );
      const ring = rest.length ? [...rest.slice(0, 3), bgc] : [bgc, bgc];
      const isRise = style === "rise";
      // Always centered; variety comes from shape, size, and squash.
      const cx = lw / 2;
      // How far past the edge the center sits.
      const edge = rand() * 0.18;
      ...
      const paintDome = (
        ...
        for (const [i, c] of ring.entries()) {
          ...
          g.addColorStop(
            Math.min(1, Math.max(0, t + jitter)),
            `rgba(${cr},${cg},${cb},${last ? 0 : alpha})`
          );
        }
        o.fillStyle = g;
        const cover = r * 1.1;
      ...
      if (rand() < 0.4 && rest.length > 1) {
        paintDome(
          cy,
          squashX * (0.55 + rand() * 0.4),
          squashY * (0.6 + rand() * 0.5),
          R * (0.4 + rand() * 0.3),
          0.55
        );
      }
    ...
    case "radial": {
      ...
      const byLum = [...cols].toSorted((a, b) => luminance(b) - luminance(a));
      ...
      if (order.length > 4) {
        const secondLast = order.at(-2);
        const last = order.at(-1);
        if (!secondLast || !last) {
          throw new Error("A radial gradient requires palette colors");
        }
        order = [order[0], order[1], secondLast, last];
      }
      ...
      const g = o.createRadialGradient(
        ...
        fx,
        fy,
        maxDim * (0.85 + rand() * 0.4)
      );
      for (const [i, c] of order.entries()) {
        const t = i / (order.length - 1 || 1);
        const jitter =
          i === 0 || i === order.length - 1 ? 0 : (rand() - 0.5) * 0.12;
        g.addColorStop(Math.min(1, Math.max(0, t + jitter)), c);
      }
      o.fillStyle = g;
      fillAll();
    ...
    default: {
      ...
      throw new Error(`Unsupported gradient style: ${exhaustiveStyle}`);
    }
  }
}
...
function cellHash(x: number, y: number, seed: number): number {
  let n = (x * 73_856_093) ^ (y * 19_349_663) ^ (seed | 0);
  n = Math.imul(n ^ (n >>> 13), 0x5b_d1_e9_95);
  return (n ^ (n >>> 15)) >>> 0;
}
...
function drawAscii(
  ...
  for (let y = 0; y < rows; y += 1) {
    for (let x = 0; x < cols; x += 1) {
      ...
      if (drive * (0.25 + s.ascii.density * 0.95) <= threshold) {
        continue;
      }
      const pool =
        pools[Math.min(pools.length - 1, Math.floor(drive * pools.length))];
      const ch = pool[cellHash(x, y, s.seed) % pool.length];
      let cb: number, cg: number, cr: number;
      if (lightBg) {
        cr = r * 0.4;
        cg = g * 0.4;
        cb = b * 0.4;
...
export function renderGradient(
  ...
  if (gtype === "pixel" || gtype === "dither") {
    ...
    const block =
      gtype === "pixel"
        ? Math.max(3, Math.round(w / 200))
        : Math.max(2, Math.round(w / 440));
    ...
    if (s.grain > 0) {
      ...
      const d = img.data;
      const nrand = mulberry32((s.seed ^ 0x51_ab_7e_55) >>> 0);
