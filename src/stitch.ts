...
import { renderGradient, type Settings } from './engine'
...
export function renderPage(
  ...
  const hs = heights ?? sections.map((s) => Math.max(2, Math.round(w / s.ratio)))
  ...
  for (let i = 0; i < sections.length; i++) {
    const h = hs[i]
    const feather = i === 0 ? 0 : Math.round(Math.min(h, hs[i - 1]) * 0.5 * Math.min(1, Math.max(0, blend)))
